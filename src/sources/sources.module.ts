 ...
 * the same HTTP settings. Adding a new site = add a spec in site-specs.ts and
 * its id to KNOWN_SOURCE_IDS — no wiring change here.
 */
const listingSourcesProvider: Provider = {
  ...
  useFactory: (config: ConfigService<AppConfig, true>) => {
    ...
    new Logger('SourcesModule').log(`sources=[${cfg.enabled.join(', ')}]`);
    return cfg.enabled
